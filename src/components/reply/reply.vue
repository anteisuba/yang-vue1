<template>
  <yk-space size="m" class="reply">
    <div class="reply__name">
      <yk-avatar
        img-url="https://pbs.twimg.com/profile_images/1759288578851708928/3vfE6G6y_400x400.jpg"
        v-if="isComment"
      />
      <div class="name-time">
        <yk-text
          ><a href="https://x.com/ndng16752248">{{ content?.user.name }}</a></yk-text
        >
  ...
  <yk-space dir="vertical" size="s" class="reply__main">
    <div class="comment">
      ...
      <img
        src="https://pbs.twimg.com/media/Gjqk9TsbwAEmbf5?format=jpg&name=4096x4096"
        alt="Phoebe fanart"
      />
    </div>
    <yk-space>
      <yk-tag type="primary">コミック</yk-tag>
      <yk-text type="success" v-show="content?.complaint! > 0"
        >いいね {{ content?.complaint }}</yk-text
      >
    </yk-space>
  </yk-space>
</template>
